...
int main(void) {
	...
	//#pragma omp parallel for private(i) shared(ROW_SIZE,NUM_OF_BLOCK,Big_Block,b,SIZE_OF_COLLECTION,SIZE_OF_COMBINATION){
	for(int i = 0; i < 498; i++){  //423 & 499 broken

		printf("THIS IS COLUMN %d\n", i);

		double*c  = readCol(jz,i,4400);

		// #pragma omp prallel for private(c,j) shared(ROW_SIZE,NUM_OF_BLOCK,Big_Block,b,SIZE_OF_COLLECTION,SIZE_OF_COMBINATION){
